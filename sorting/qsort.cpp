...
*/
void two_of_five(int* first, int* last) {
	auto c1 = first;
	ptrdiff_t dist = last - first + 1;
	ptrdiff_t quarter = dist / 4;
...
Returns:
	middle interval
*/
std::pair<int*, int*> three_way_partition(int* first, int* last) {
	int p1 = *first;
	int p2 = *last;
	...
	return std::make_pair(lessPtr, greaterPtr);
}


void dpsort(int* first, int* last) {
	if (last-first >= 17) {
		two_of_five(first, last-1);
		auto mid = three_way_partition(first, last-1);
		dpsort(first, mid.first);
		dpsort(mid.first, mid.second+1);
