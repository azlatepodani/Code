...
void radix_pass_recurse(RandomIt first, RandomIt last, ExtractKey&& ek, NextSort&& continuation)
{
	counters_t count = {0};
...
//  1. The range [first, last) will have less than INT_MAX elements
//
inline void radix_string(std::string* first, std::string* last, int round) {
	radix_pass_recurse(first, last, ExtractStringChar(round),
...
//  1. The range [first, last) will have less than INT_MAX elements
//
inline void radix_string(std::wstring* first, std::wstring* last, int round)
{
	radix_pass_recurse(first, last, compose_ch(ExtractHighByte(), ExtractStringChar(round)),
		[round](std::wstring* first, std::wstring* last) {
			radix_pass_recurse(first, last, compose_cl(ExtractLowByte(), ExtractStringChar(round)),
				[round](std::wstring* first, std::wstring* last) {
					radix_string(first, last, round+1);
				});
		});
